...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
   // The function row is gone. Grave Escape holds the top-left seat: tap = Esc,
   // Shift = ~. A bare ` lives on _FN1 at the same key, and F1~F12 on _FN1's
   // number row -- dropping the row costs no keycode.
   [_QWERTY] = LAYOUT(
      ...
      KC_LCTL,  KC_LGUI, KC_LALT, MO(_FN2),KC_SPC,                              KC_RALT, KC_SPC,  MO(_FN1),KC_INS,  KC_DEL,  KC_LEFT, KC_DOWN, KC_RGHT),
   [_FN1] = LAYOUT(
